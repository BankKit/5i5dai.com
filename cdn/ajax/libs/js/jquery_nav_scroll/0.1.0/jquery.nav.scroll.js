 ...
 * Version: 0.1.0-alpha
 *
 * Creation Date: 2014.01.17 17:51 ( Tony ).
 *
 * Last Update: 2014.01.17 17:55 ( Tony ).    ...//TODO: Update the 'Last Update'.
 *
		...
		(function ($) {
			...
			var S = {
				...
				_setup_nav: function (sections) {
					...
					var $headline = $('<span />', {
						'class': 'scroll-nav__heading',
						text: S.settings.headlineText
					});
					var $wrapper = $('<div />', {
						'class': 'scroll-nav__wrapper'
					});
					var $nav = $('<nav />', {
						'class': 'scroll-nav',
						'role': 'navigation'
					...
					var $nav_list = $('<ol />', {
						'class': 'scroll-nav__list'
					});

					$.each(sections, function (i) {
						var $item = (i === 0) ? $('<li />', {
							'class': 'scroll-nav__item active'
						}) : $('<li />', {
						...
						if (this.sub_sections.length > 0) {
							...
							$sub_nav_list = $('<ol />', {
								'class': 'scroll-nav__sub-list'
							});

							$.each(this.sub_sections, function () {
								var $sub_item = $('<li />', {
									'class': 'scroll-nav__sub-item'
								});
								var $sub_link = $('<a />', {
									'href': '#' + this.id,
									'class': 'scroll-nav__sub-link',
									text: this.text
								});
